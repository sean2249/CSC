...
shape_SIGHAN = pd.read_csv(filelist['Bakeoff2013_CharacterSet_SimilarShape.txt'],                            sep=',', index_col=0, names=['cands']).to_dict()['cands']
...
sorted(confus.items(), key=lambda x:x[1][1], reverse=True)[:10]
...
tmp = sorted(ttttt, key=lambda x:x[1][0], reverse=True)
...
tmp_score  = [(val) for val in tmp_t]
...
with open('./extractUDN/udn_pairs.csv', 'r') as fp:
